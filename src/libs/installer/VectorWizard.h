...
public:
	...
	VectorWizardPage *page(int id) const;
	bool hasVisitedPage(int id) const;
	QList<int> visitedPages() const;    // ### Qt 6: visitedIds()?
	QList<int> pageIds() const;
	void setStartId(int id);
	int startId() const;
	VectorWizardPage *currentPage() const;
	int currentId() const;
...
private:
	Q_DISABLE_COPY(VectorWizard)
	Q_DECLARE_PRIVATE(VectorWizard)
	...
	friend class VectorWizardPage;
};

Q_DECLARE_OPERATORS_FOR_FLAGS(VectorWizard::WizardOptions)
...
public:
	explicit VectorWizardPage(QWidget *parent = Q_NULLPTR);
	~VectorWizardPage();
	...
	void setPixmap(VectorWizard::WizardPixmap which, const QPixmap &pixmap);
	QPixmap pixmap(VectorWizard::WizardPixmap which) const;
	void setFinalPage(bool finalPage);
	bool isFinalPage() const;
	void setCommitPage(bool commitPage);
	bool isCommitPage() const;
	void setButtonText(VectorWizard::WizardButton which, const QString &text);
	QString buttonText(VectorWizard::WizardButton which) const;
...
protected:
	...
	VectorWizard *wizard() const;

private:
	Q_DISABLE_COPY(VectorWizardPage)
	Q_DECLARE_PRIVATE(VectorWizardPage)
	Q_PRIVATE_SLOT(d_func(), void _q_maybeEmitCompleteChanged())
	Q_PRIVATE_SLOT(d_func(), void _q_updateCachedCompleteState())

	friend class VectorWizard;
	friend class VectorWizardPrivate;
};
